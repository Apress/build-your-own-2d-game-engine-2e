...
vec4 HLightEffect(Light lgt) {
    vec4 result = vec4(0);
    // HACK: assume 
    //      Near defines Oscillation frequency results the W of 2*PI*W
    //      Far  defines distance   
    //   On/Off  controls damping
    float dist = length(lgt.Position.xyz - gl_FragCoord.xyz);
    float frac = dist / lgt.Far;
    float strength = 0.0;
    float damp = 1.0;
    if (frac < 1.0) {
        float theta = frac * lgt.Near * 2.0 * 3.1416;
        if (lgt.IsOn)
            damp = (1.0 - frac) * (1.0 - frac);
        strength = damp * 0.5 * ( 1.0 + cos(theta));
    }
...
void main(void)  {
    ...
    if (textureMapColor.a > 0.0) {
        lgtResults += HLightEffect(uLights[0]);
        for (int i=1; i<kGLSLuLightArraySize; i++) { 
            if (uLights[i].IsOn) { 
